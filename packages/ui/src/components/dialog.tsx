...
>(({ className, children, full, size = 'sm', onClose = () => {}, ...props }, ref) => (
  <DialogPortal>
    <DialogOverlay onClick={onClose} />
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-[50%] top-[50%] z-[10000] flex flex-col translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%] rounded-lg sm:rounded-lg',
        full
          ? 'w-[95vw] h-[95vh] lg:h-[90vh]'
          : cn(
              'w-full max-w-[95vw] h-auto min-h-[100px]',
              size === 'sm' && 'sm:max-w-lg',
...
const DialogFooter = ({
  ...
  <div
    className={cn(
      'mt-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-2',
      className,
    )}
    {...props}
  />
