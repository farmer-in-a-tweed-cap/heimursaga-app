    ...
    switch (template) {
      ...
      case EMAIL_TEMPLATES.PASSWORD_RESET:
        emailData = getEmailTemplate(EMAIL_TEMPLATES.PASSWORD_RESET, {
          reset_link:
            'https://heimursaga.com/reset-password?token=sample-token-123',
        });
        break;
      ...
      case EMAIL_TEMPLATES.EMAIL_VERIFICATION:
        emailData = getEmailTemplate(EMAIL_TEMPLATES.EMAIL_VERIFICATION, {
          verification_link:
            'https://heimursaga.com/verify-email?token=sample-verification-token-123',
        });
        break;
      ...
      case EMAIL_TEMPLATES.EXPLORER_PRO_NEW_ENTRY:
        emailData = getEmailTemplate(EMAIL_TEMPLATES.EXPLORER_PRO_NEW_ENTRY, {
          ...
          postContent:
            'After hiking for 3 days through challenging terrain, we finally reached the summit just as the sun began to rise. The view was absolutely breathtaking - golden light spreading across the mountain peaks as far as the eye could see.\n\nThe temperature was -15°C, but the warmth from this incredible sight made every frozen step worth it. This is why I explore - for moments like these that remind you how beautiful our world truly is.',
          ...
          postWaypoint: { lat: 27.9881, lon: 86.925 },
          mapUrl:
            'https://via.placeholder.com/600x300/45B7D1/FFFFFF?text=Map+Location',
          postUrl: 'https://heimursaga.com/entries/sample-entry-123',
          unsubscribeUrl:
            'https://heimursaga.com/unsubscribe?token=sample-token',
